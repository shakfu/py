 ...
 * 
 * Usage:
 *   #include "pysmart_cache_instance.h"
 * 
 *   // Create instance
...
#ifndef PYSMART_CACHE_INSTANCE_H
#define PYSMART_CACHE_INSTANCE_H
 ...
 * @param instance Cache instance to destroy
 */
...
static inline psc_result_t psc_add_function(psc_instance_t *instance, const char *source_code, const char *filename) {
    ...
    if (!should_cache) {
        ...
        if (instance->config.debug_mode) {
            printf("PSC[%s]: Skipping function - %s\n", 
    ...
    strncpy(function_name, instance->workspace.temp_function_name, sizeof(function_name) - 1);
...
/**
 * @brief Clean up the smart caching system instance
...
#endif // PYSMART_CACHE_INSTANCE_H
